 ...
 * Assumes fast-vector dynamics
 * @author David
 */

public class EpiModelDengueFastVectored extends EpiModel {
    ...
    public void setInitParams()
    {
        ...
        betaHV = 0.2475; params.add(betaHV); estParams.add(0);
        betaVH = betaHV; params.add(betaVH); fixedParams.add(1);
        betaGF = 0.0; params.add(betaGF); fixedParams.add(2);
        betaGG = 0.16; params.add(betaGG); fixedParams.add(3);
        betaFG = betaGF; params.add(betaFG); fixedParams.add(4);
        
        alphaFocal = 0.08; params.add(alphaFocal); estParams.add(5);
        ...
        NFocal = 2.0e06; params.add(NFocal); fixedParams.add(9);
        NGlobal = 25.0e06; params.add(NGlobal); fixedParams.add(10);
        mu = 1/(70*365.25); params.add(mu); fixedParams.add(11);
        nu = 1.0/7.0; params.add(nu); fixedParams.add(12);
        
        //Additional vector params;
        M = 1.0; params.add(M); estParams.add(13);
        muVec = 1.0 / 7.0; params.add(muVec); fixedParams.add(14);
        
        //Fix initSFocal
        //initSFocal = 0.40; params.add(initSFocal); estParams.add(15); //estimated as the fraction, not the absolute number
    ...
    public DoubleArrayList getInitialConditions() 
    {
        ...
        //double initI1 = mu * NFocal * (R0Focal - 1) / ((betaHV+betaVH)/2.0); currInits.add(initI1);
	...
	for (int n = 0; n < xDtTimes.size(); ++n) {
            ...
            for (int j = 0; j < jParticles; ++j) {
                ...
                dSfIf = (betaVH * matrix.getQuick(0,j,xIndex) * vecI * dt / matrix.getQuick(4,j,xIndex));
	...
	for (int n = 0; n < times.size(); ++n) {
            ...
            double vecI = (seasB * betaHV * (x.getQuick(2) / NFocal)) / ((betaHV/NFocal)*x.getQuick(2)*muVec + muVec*muVec);
            ...
            dSfIf = (betaVH * x.getQuick(0) * vecI * dt / x.getQuick(4));
